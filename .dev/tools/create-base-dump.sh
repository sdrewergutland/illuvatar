...
mysqldump -h"${DATABASE_HOST}" -u"${DATABASE_USER}" -p"${DATABASE_PASSWORD}" --no-data --add-drop-table --single-transaction ${DATABASE_NAME} >tests/Resources/Sql/base.default.structure.sql
mysqldump -h"${DATABASE_HOST}" -u"${DATABASE_USER}" -p"${DATABASE_PASSWORD}" --no-create-info --compact ${DATABASE_NAME} > tests/Resources/Sql/base.default.data.sql
...
mysqldump -h"${DATABASE_HOST}" -u"${DATABASE_USER}" -p"${DATABASE_PASSWORD}" --add-drop-table --no-data ${DATABASE_NAME} $(mysql -u${DATABASE_USER} -p${DATABASE_PASSWORD} -h${DATABASE_HOST} -Ne"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='${DATABASE_NAME}' AND TABLE_TYPE='BASE TABLE'") \
  | grep ^DROP \
